...
pygame.display.set_caption("Raspi Light Show Recorder")
...
def update(events, keys):
    ...
    pos_text = status_font.render(str(pygame.mouse.get_pos()) + " timer: " + str(timer / 60), True, (233, 244, 244))
    ...
    for box in indicators:
        if keys[box['key'][1]]:
            ...
            if recorded_data.get(timer) is not None:
                if box['key'] not in recorded_data[timer]:
                    recorded_data[timer].append(box['key'])
            else:
                recorded_data[timer] = box['key']
        else:
            if recorded_data.get(timer) is not None:
                pass
            else:
